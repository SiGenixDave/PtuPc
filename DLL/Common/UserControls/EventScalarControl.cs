 ...
 *  Project:    Common
 * 
 *  File name:  EventScalarControl.cs
 * 
 *  Revision History
 *  ----------------
 * 
...
namespace Common.UserControls
{
    ...
    /// </summary>
    public partial class EventScalarControl : EventControl
    {
        #region --- Constructors ---
        /// <summary>
        /// Initializes a new instance of th class.
        /// </summary>
        public EventScalarControl()
        {
            InitializeComponent();
        }
            ...
            set
            {
                ...
                else
                {
                    ...
                    if (m_LabelNameField.Text != CommonConstants.VariableNotDefinedString)
                    {
                        ...
                        try
                        {
                            engineeringValue = m_Value * Lookup.EventVariableTable.Items[m_Identifier].ScaleFactor;
                            engineeringValue = Math.Round(engineeringValue, m_DecimalPlaces);

                            if (Lookup.EventVariableTable.Items[m_Identifier].FormatString.ToLower() == FormatStringFieldGeneralNumber)
                            {
                                m_LabelValueField.Text = engineeringValue.ToString(FormatStringNumericString);
                            }
                            else if (Lookup.EventVariableTable.Items[m_Identifier].FormatString.ToLower() == FormatStringFieldHexadecimal)
                            {
